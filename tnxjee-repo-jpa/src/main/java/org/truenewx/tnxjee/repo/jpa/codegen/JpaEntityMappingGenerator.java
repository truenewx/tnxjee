...
public class JpaEntityMappingGenerator extends ModelBasedGenerator {
    ...
    public JpaEntityMappingGenerator(String modelBasePackage) {
    ...
    public void generate(String... modules) throws Exception {
        if (ArrayUtils.isEmpty(modules)) {
            Collection<Class<?>> classes = ClassUtil.findClasses(this.modelBasePackage, Entity.class);
            for (Class<?> clazz : classes) {
                Class<? extends Entity> entityClass = (Class<? extends Entity>) clazz;
                String tableName = "t_" + StringUtil.prependUnderLineToUpperChar(clazz.getSimpleName(), true);
                generate(entityClass, tableName);
            }
        } else {
            for (String module : modules) {
                String basePackage = this.modelBasePackage + Strings.DOT + module;
                Collection<Class<?>> classes = ClassUtil.findClasses(basePackage, Entity.class);
                for (Class<?> clazz : classes) {
                    Class<? extends Entity> entityClass = (Class<? extends Entity>) clazz;
                    String tableName = "t_" + StringUtil.prependUnderLineToUpperChar(clazz.getSimpleName(), true);
    ...
    private Binate<Boolean, String> getBasicConverter(Class<?> fieldType) {
    ...
    private String getConverter(Class<?> fieldType) {
